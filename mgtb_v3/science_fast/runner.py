...
def run_role(
    *, settings: dict[str, Any], manifest: dict[str, Any], role: str, method: str, output_dir: str | Path,
    ...
    if calibrator_payload is None:
        ...
        calibrator = PositionalCalibrator(calibrator_payload["buckets"], calibrator_payload["score_pools_by_bucket"], calibrator_payload["p_clip"])
    threshold_factor = math.inf if method == "vanilla" else math.exp(float(selected_h))
    source = {
    ...
    def worker(item: dict[str, Any]) -> dict[str, Any]:
        ...
        trace_path = Path(output_dir) / "in_progress" / (RunStore.filename(item["item_id"]) + ".trace.jsonl")
        started = time.perf_counter()
        result = generate_with_mgtb_v3(
            model, tokenizer, format_math500_prompt(item["problem"]), controller, calibrator, threshold_factor,
            max_new_tokens=int(settings["max_new_tokens"]), trace_log_path=trace_path, do_backtracking=(method == "mgtb"),
        )
        wall = time.perf_counter() - started
        prompt_ids = tokenizer(format_math500_prompt(item["problem"]), return_tensors="pt")["input_ids"][0].tolist()
        ...
        return {
            ...
            "token_accounting": accounting, "monitor_trace": trace if method == "mgtb" or role in {"reference", "development"} else [],
            ...
            "provenance": {**source, "run_identity_sha256": store.identity_sha256, "resolved_config_sha256": identity["resolved_config_sha256"]},
        }

    return store.run(manifest["roles"][role], worker, stop_after=stop_after)
